...
## Flex layout
모든 웹사이트는 커다란 레이아웃 형태의 구조로 되어있고, 이 레이아웃을 만드는 방법은 다양합니다. 옛날에는 `<table>` 태그와 `<div>` 태그 만으로 노가다 형태로 사이트 구조를 만드는 경우들이 많았는데, 최근에는 Flext layout을 통해 css 기능을 다양하게 상요할 수 있습니다.
